...
namespace Microsoft.ML.Ensemble.OutputCombiners
{
    internal abstract class BaseStacking<TOutput> : IStackingTrainer<TOutput>
    {
        ...
        private void CheckMeta()
        {
            ...
            if (!(ivm.InputType is VectorType vectorType) || vectorType.ItemType != NumberDataViewType.Float)
        ...
        public void Train(List<FeatureSubsetModel<TOutput>> models, RoleMappedData data, IHostEnvironment env)
        {
            ...
            using (var ch = host.Start("Training stacked model"))
            {
                ...
                bldr.AddColumn(DefaultColumnNames.Label, NumberDataViewType.Float, labels);
                bldr.AddColumn(DefaultColumnNames.Features, NumberDataViewType.Float, features);
