...
namespace Microsoft.ML.Tests.Transformers
{
    public class KeyToVectorEstimatorTest : TestDataPipeBase
    {
        ...
        private void ValidateMetadata(IDataView result)
        {
            ...
            Assert.Equal(column.Annotations.Schema.Select(x => x.Name), new string[1] { MetadataUtils.Kinds.SlotNames });
            ...
            Assert.Equal(column.Annotations.Schema.Select(x => x.Name), new string[3] { MetadataUtils.Kinds.SlotNames, MetadataUtils.Kinds.CategoricalSlotRanges, MetadataUtils.Kinds.IsNormalized });
            column.GetSlotNames(ref slots);
            Assert.True(slots.Length == 1);
            Assert.Equal(slots.Items().Select(x => x.Value.ToString()), new string[1] { "C" });
            column.Annotations.GetValue(MetadataUtils.Kinds.CategoricalSlotRanges, ref slotRanges);
            ...
            Assert.Equal(column.Annotations.Schema.Select(x => x.Name), new string[2] { MetadataUtils.Kinds.SlotNames, MetadataUtils.Kinds.IsNormalized });
            column.GetSlotNames(ref slots);
            Assert.True(slots.Length == 2);
            ...
            Assert.Equal(column.Annotations.Schema.Select(x => x.Name), new string[2] { MetadataUtils.Kinds.CategoricalSlotRanges, MetadataUtils.Kinds.IsNormalized });
            column.Annotations.GetValue(MetadataUtils.Kinds.CategoricalSlotRanges, ref slotRanges);
            Assert.True(slotRanges.Length == 4);
            ...
            Assert.Equal(column.Annotations.Schema.Select(x => x.Name), new string[1] { MetadataUtils.Kinds.IsNormalized });
            Assert.True(column.IsNormalized());
